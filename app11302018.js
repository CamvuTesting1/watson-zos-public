...
function pollJCLJobStatus(deferred, client, jobName, jobId, timeOutCount) {
	...
	if (!response.output) {
		...
		if (response.output.generic[0].text.indexOf('BA55054')>=0) {
			print = 1;
			response.output.generic[0].text = response.output.text + 'generic 0';
		}
		else if (response.output.generic[1].text.indexOf('We')>=0)
			response.output.generic[1].text = response.output.generic[1].text + 'generic 1';
		return response;
	}
