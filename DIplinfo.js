...
function DIplinfoNativeSys(DIplinfoAction, callback) {
	var HOST = DIplinfoAction.parameters.systemIP;
...
function SubmitDIplinfoJob(client, JCLjob, callback) {
	...
	submitJob(client, JCLjob).then(function (result) {
		...
		console.log('SubmitDIplinfoJob: ' + result.jobName + ' JobID...' + result.jobId);
		...
		console.log('DIplinfoNativeSysa(DIplinfoAction): returned an error');
		console.dir(error);
...
function parsing(jobString) {
	...
	for (var i = 0; i < linesArray.length; i++) {
		...
		if (linesArray[i].indexOf('IPL DEVICE') >= 0) {
